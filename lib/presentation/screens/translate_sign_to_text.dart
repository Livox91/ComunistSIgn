...
class _TranslateSignToTextScreenState extends State<TranslateSignToTextScreen>
  ...
  // LSTM phrase predictions (top-3 chips shown when buffer fills)
  List<PhrasePrediction> _phrasePredictions = const [];
  ...
  void _applyGesture(String gesture) {
    ...
    // Same letter cooldown: ignore if the hand hasn't changed since last append.
    // Allow deliberate repetition only after 2 seconds (e.g. typing "LL").
    if (gesture == _lastAppended &&
        now.difference(_lastAppendTime).inMilliseconds < 2000) {
      return;
    }
  ...
  void _startDetection() {
    ...
    _detectionTimer = Timer.periodic(Duration(milliseconds: 1000), (_) async {
      ...
      try {
        ...
        final r = await _backendService!.sendFrameToBackend(bytes);
        if (!mounted) return;
        ...
        setState(() {
          ...
          // Top-3 phrase predictions (shown as tappable chips below)
          _phrasePredictions = r.phrasePredictions;

          // Legacy phrase string-matcher fires occasionally — show + reset
          if (r.phrase != null && r.phrase!.isNotEmpty &&
              r.phrase != 'Detecting...') {
            _statusText = r.phrase!;
          } else if (r.gestures.isEmpty) {
            _statusText = 'Detecting...';
          } else {
  ...
  void _clearText() {
    setState(() {
      _currentWord = '';
      _completedSentence = '';
      _phrasePredictions = const [];
      _statusText = 'Cleared';
    });
  }

  /// User tapped a phrase prediction chip — append it to the sentence.
  void _acceptPhrase(String phrase) {
    setState(() {
      if (_currentWord.isNotEmpty) {
        _completedSentence = '$_completedSentence$_currentWord ';
        _currentWord = '';
      }
      _completedSentence = '$_completedSentence$phrase ';
      _phrasePredictions = const [];
    });
  }
  ...
  Widget _buildPhraseChips() {
    if (_phrasePredictions.isEmpty) return SizedBox.shrink();
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      alignment: WrapAlignment.center,
      children: _phrasePredictions
          .map((p) => ActionChip(
                avatar: Icon(Icons.translate, size: 16, color: Colors.white),
                label: Text(
                ...
                onPressed: () => _acceptPhrase(p.label),
              ))
          .toList(),
    );
  }
                        ...
                        child: SingleChildScrollView(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_phrasePredictions.isNotEmpty) ...[
                                SizedBox(height: 10),
                                _buildPhraseChips(),
                              ],
                            ],
